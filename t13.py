...
def main():
    ...
    model = YOLO('yolov9s.pt')  # Use YOLOv8 small model
    ...
    # Timer to limit frame processing
    last_process_time = time.time()

    try:
        while True:
            # Limit processing to every 3 seconds
            if time.time() - last_process_time < 3:
                continue
            last_process_time = time.time()
            ...
            detections = results[0].boxes  # Get YOLOv8 box results
            bounding_boxes = detections.xyxy.cpu().numpy()  # Bounding boxes
            confidences = detections.conf.cpu().numpy()  # Confidence scores
            classes = detections.cls.cpu().numpy()  # Class labels

            # Combine detections into a single array
            detections = np.hstack((bounding_boxes, confidences[:, None], classes[:, None]))
            ...
            # Filter depth and color data based on YOLOv8 detections
...
def annotate_detections(image, detections):
    """
    Annotates the image with bounding boxes and class labels from YOLOv8 detections.
    """
